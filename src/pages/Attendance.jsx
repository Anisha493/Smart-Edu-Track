...
export default function Attendance() {
  ...
  const { data: existing = [] } = useQuery({
    ...
    queryFn: () =>
      base44.entities.Attendance.filter({ class_id: classId, date }),
  ...
  useEffect(() => {
    const map = {};
    existing.forEach((r) => {
      map[r.student_id] = { status: r.status, id: r.id };
    });
    students.forEach((s) => {
      if (!map[s.id]) map[s.id] = { status: "present" };
    });
    setMarks(map);
  }, [existing, students]);
  ...
  return (
    <div>
      <PageHeader
        title="Attendance"
        subtitle="Mark attendance quickly and accurately"
      />
      ...
      {!classId ? (
        <Card>
          <EmptyState
            ...
            description="Pick a class and date above to mark attendance"
          />
        </Card>
      ) : students.length === 0 ? (
        <Card>
